    ...
    methods: {
        scrolling: function (e) {
            ...
            me.data.timer = setTimeout(function () {
                var _a = me.data, minDate = _a.minDate, maxDate = _a.maxDate;
                var monthData = me.data.monthData;
                if (!(monthData === null || monthData === void 0 ? void 0 : monthData.length)) {
                    monthData = (0, utils_1.getMonths)(minDate, maxDate);
                }
                var _b = e.detail, scrollTop = _b.scrollTop, scrollHeight = _b.scrollHeight;
                //  每一行的高度。
                var cHeight = scrollHeight / (monthData.length);
                //  当前所在月
                var curMonth = Math.floor(scrollTop / cHeight);
                var monthHideMap = {};
                for (var i = Math.max(0, curMonth - 1); i < Math.min(monthData.length, curMonth + 3); i++) {
                    monthHideMap[monthData[i]] = true;
                }
                me.setData({
                    // @ts-ignore
                    monthData: monthData,
                    monthHideMap: monthHideMap,
                });
        ...
        initRect: function () {
            ...
            var _b = this.data, minDate = _b.minDate, maxDate = _b.maxDate, maxSimDays = _b.maxSimDays;
            if (minDate && maxDate && Math.abs(maxDate - minDate) > maxSimDays * 24 * 60 * 60 * 1000) {
                ...
                if (!((_a = Object.keys(this.data.monthHideMap)) === null || _a === void 0 ? void 0 : _a.length)) {
                    for (var i = 0; i < Math.min(3, monthData.length); i++) {
                        monthHideMap[monthData[i]] = true;
                    }
                }
                this.setData({
